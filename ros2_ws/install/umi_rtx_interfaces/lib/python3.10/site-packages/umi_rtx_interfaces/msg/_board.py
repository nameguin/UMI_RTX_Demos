...
class Board(metaclass=Metaclass_Board):
    ...
    SLOT_TYPES = (
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('int32'), 9),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        if 'data' not in kwargs:
            self.data = numpy.zeros(9, dtype=numpy.int32)
        else:
            self.data = numpy.array(kwargs.get('data'), dtype=numpy.int32)
            assert self.data.shape == (9, )
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.int32, \
                    "The 'data' numpy.ndarray() must have the dtype of 'numpy.int32'"
                assert value.size == 9, \
                    "The 'data' numpy.ndarray() must have a size of 9"
                self._data = value
                return
            ...
            assert \
                ...
                "The 'data' field must be a set or sequence with length 9 and each value of type 'int' and each integer in [-2147483648, 2147483647]"
        self._data = numpy.array(value, dtype=numpy.int32)
